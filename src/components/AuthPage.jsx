  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response) {
        ...
        const isLocalhostApi = axiosInstance.defaults.baseURL.includes('localhost');
        const isProdEnv = window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1';
        
        if (isLocalhostApi && isProdEnv) {
          setError('Deployment Configuration Error: The app is trying to connect to a local server ("localhost") from a live website. Please set your VITE_API_URL in Vercel environment variables.');
        } else {
          setError('Backend Unreachable: Unable to reach the server. Please check your internet or verify if the backend is running.');
        }
      } else {
